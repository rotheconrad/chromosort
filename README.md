...
| `chromo eval` | Prepare editable TSV review tables for algorithm-assisted, human-reviewed decisions, including `chromo eval fix` tables for `chromo fix --reviewed-plan` and `chromo eval scaffold` tables for `chromo scaffold --reviewed-plan` ([eval docs](https://rotheconrad.github.io/chromosort/commands/eval/)). |
