...
| Submission checklist model | `scaffold` and `gapfill` | Emitted automatically beside scaffold-like outputs | `submission_checklist.tsv` rows for file manifest, FASTA hygiene, AGP continuity, FASTA/AGP consistency, unresolved gaps, and graph-fill spans | `tests/test_scaffold.py`, `tests/test_gapfill.py` |
...
| `submission.py` | Shared submission-oriented checklist rows for scaffold-like FASTA/AGP outputs, including file manifests, FASTA hygiene, AGP consistency, unresolved gap counts, and graph-fill span summaries. |
