...
| `chromo eval gapfill` | `evaluate.py` and `gapfill.py` | Ordered FASTA plus assignment TSV, or scaffold FASTA plus AGP; GFA; optional projected GFA `P`/`W` paths, GAF, Hi-C table, reference-placement PAF, long-read PAF, and external patch table/FASTA | Prepare an editable review-event TSV from the gapfill planner, including direct or projected candidate path status, fillability, branch-support evidence, read-bridge context, and patch concordance. | Does not change FASTA. Accepted rows can later restrict `chromo gapfill --reviewed-plan --apply`; projected no-sequence rows and external patch candidates remain review-only. |
| `chromo eval all` | `evaluate.py`, `fix_contigs.py`, `scaffold.py`, and `gapfill.py` | Fix inputs plus ordered FASTA, assignment TSV, GFA, and optional shared read/graph evidence | Run the existing fix, scaffold, and gapfill review-table paths from one input bundle and write a small manifest with the recommended `gafprep` table arguments. | Does not change FASTA. The three emitted tables still feed their matching executors. |
| `chromo fix` | `fix_contigs.py` | Assembly FASTA, coords or PAF, selected contigs or `--all`, optional GFA | Detect and split selected chimeric contigs into reference-labeled pieces using mode-specific breakpoint planning. | Writes a fixed FASTA, split report, and AGP/component/checklist sidecars. Optionally writes only pieces and/or reverse-complements pieces. |
| `chromo cut` | `cut.py` | Assembly FASTA plus explicit cut positions | Apply reviewed 1-based cut positions and report emitted pieces. | Writes a cut FASTA, cut report, and AGP/component/checklist sidecars. No cut is inferred from alignments. |
...
| `chromo manual apply` | `manual.py` | Assembly FASTA and exported recipe JSON | Apply a dashboard recipe containing slices, orientation choices, removals, and optional scaffold grouping. | Writes a manually edited FASTA, optional report, and AGP/component/checklist sidecars. |
...
| Review-event TSV model | `eval fix/scaffold/gapfill/all`, `manual fix/scaffold/gapfill`, `fix --reviewed-plan`, `scaffold --reviewed-plan`, `gapfill --reviewed-plan` | `--review-table` in manual modes; `--reviewed-plan` in executors; `eval all --output-prefix` for the three-table bundle | `*.fix_review.tsv`, `*.scaffold_review.tsv`, `*.gapfill_review.tsv`, and `*.eval_all_outputs.tsv` using schema `chromosort-review-event-v1` for review rows | `tests/test_review.py`, `tests/test_eval.py`, executor tests |
...
| Targeted GraphAligner input preparation | `gafprep`; commonly seeded by `eval all` review tables | `--target-padding`, `--contig-end-window`, `--target-reads-per-interval`, `--background-*`, `--max-reads*`, GraphAligner script options | `*.targets.tsv`, `*.selected_reads.tsv`, `*.selected_read_review_links.tsv`, `*.selected.fastq.gz`, `*.graphaligner.gfa`, `*.graphaligner.sh`, sanitization reports | `tests/test_gafprep.py`, `tests/test_cli.py` |
...
| External patch candidate comparison | `gapfill --patch-table` and `eval gapfill --patch-table` | `--patch-fasta`, `--include-fill-sequences` | `patch_*` plan/review fields and `multiple_external_patches`, `external_patch_graph_mismatch`, or `external_patch_only` risk flags | `tests/test_gapfill.py`, `tests/test_eval.py` |
| FASTA provenance sidecars | `sort`, `clean`, `fix`, `cut`, `manual apply`, `scaffold`, and `gapfill` | Emitted automatically beside FASTA-changing outputs, with path overrides where commands expose sidecar flags | AGP 2.1 files plus component-provenance TSVs that map each output FASTA coordinate system back to source components or inserted gaps | `tests/test_reference_order.py`, `tests/test_clean.py`, `tests/test_fix_contigs.py`, `tests/test_cut.py`, `tests/test_manual.py`, `tests/test_scaffold.py`, `tests/test_gapfill.py` |
| Submission checklist model | `sort`, `clean`, `fix`, `cut`, `manual apply`, `scaffold`, and `gapfill` | Emitted automatically beside FASTA-changing outputs | `submission_checklist.tsv` rows for file manifest, FASTA hygiene, AGP continuity, FASTA/AGP consistency, unresolved gaps, and graph-fill spans | `tests/test_reference_order.py`, `tests/test_clean.py`, `tests/test_fix_contigs.py`, `tests/test_cut.py`, `tests/test_manual.py`, `tests/test_scaffold.py`, `tests/test_gapfill.py` |
...
| External patch concordance | `patch_candidate_count`, `patch_graph_status`, and best patch metadata | Reports whether imported patch sequence agrees with the graph-derived fill; it is not an apply decision by itself. |
...
- AGP and component-provenance sidecars map each FASTA-changing command's
  output coordinate system back to source components or inserted gaps. They are
  written fresh for each stage rather than appended across stages.
- `submission_checklist.tsv` records file-manifest rows, FASTA character and
  terminal-N checks, AGP continuity and FASTA/AGP consistency checks,
  unresolved gap totals, and graph-filled span totals.
...
| `evaluate.py` | Table-only review-event generation for `eval fix`, `eval scaffold`, `eval gapfill`, and `eval all`, reusing executor planners and adding optional graph, long-read PAF, and GAF evidence fields. |
...
| `submission.py` | Shared submission-oriented checklist rows for FASTA-changing outputs with AGP sidecars, including file manifests, FASTA hygiene, AGP consistency, unresolved gap counts, and graph-fill span summaries. |
...
- `eval fix`, `eval scaffold`, `eval gapfill`, and `eval all` table
  generation, including reviewed executor handoff cases and the all-mode
  `gafprep` manifest.
