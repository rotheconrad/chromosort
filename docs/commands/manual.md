...
`manual apply` writes a fresh AGP, component TSV, and submission checklist for
the FASTA it emits. In scaffold mode, active pieces become AGP component rows
and inserted N gaps become AGP gap rows. The default manual scaffold gap
metadata is AGP `contig/no/na`; set `--agp-gap-type`, `--agp-linkage`, and
`--agp-linkage-evidence` explicitly when you have evidence for a different NCBI
gap category.
...
| `chromo manual apply --agp` / default `<output-fasta>.agp` | AGP 2.1 map for the manual-apply FASTA. |
...
### `chromo manual apply` Parameters

| Parameter | Default | Meaning |
| --- | ---: | --- |
| `--assembly-fasta` | required | Original assembly FASTA referenced by the browser recipe. |
| `--recipe` | required | Recipe JSON exported from the manual dashboard. |
| `--output-fasta` | required | FASTA generated by applying the recipe. |
| `--report` | none | Optional TSV report of emitted and removed pieces. |
| `--agp` | `<output-fasta>.agp` | Output AGP 2.1 map for the manual-apply FASTA. |
| `--components` | `<output-fasta>.components.tsv` | Output component-provenance TSV mirroring the manual AGP rows. |
| `--submission-checklist` | `<output-fasta>.submission_checklist.tsv` | Output FASTA/AGP handoff and file-manifest checklist. |
| `--scaffold` / `--no-scaffold` | recipe export mode | Override whether active pieces are written as scaffold records or separate records. |
| `--gap-bp` | recipe gap size | Override the N-gap size inserted between pieces in scaffold mode. |
